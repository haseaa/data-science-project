import pickle
import os
from util import merge_and_sort_posts_and_tfs
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        """
        # TODO
        encode_postings_list = self.postings_encoding.encode(postings_list)
        encode_tf_list = self.postings_encoding.encode_tf(tf_list)
        current = self.index_file.tell()
        self.index_file.seek(current)
        self.terms.append(term)
        posts_tfs1 = []
        for i in postings_list:
            if i in self.doc_length:
                posts_tfs1.append((i, self.doc_length[i]))
        posts_tfs2 = [(postings_list[i], tf_list[i]) for i in range(len(postings_list))]
        merge_posts_tfs = merge_and_sort_posts_and_tfs(posts_tfs1, posts_tfs2)
        for i in range(len(merge_posts_tfs)):
            self.doc_length[merge_posts_tfs[i][0]] = merge_posts_tfs[i][1]
        self.postings_dict[term] = (current, len(postings_list), len(encode_postings_list), len(encode_tf_list))
        self.index_file.write(encode_postings_list)
        self.index_file.write(encode_tf_list)
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', postings_encoding=VBEPostings, directory='./tmp/') as index:
        index.append(1, [2, 3, 4, 8, 10], [2, 4, 129, 3, 30])
        index.append(2, [3, 4, 5], [34, 23, 56])
        index.index_file.seek(0)
        assert index.terms == [1, 2], "terms salah"
        assert index.doc_length == {
            2: 2, 3: 38, 4: 152, 5: 56, 8: 3, 10: 30}, "doc_length salah"
        assert index.postings_dict == {1: (0,
                                           5,
                                           len(VBEPostings.encode(
                                               [2, 3, 4, 8, 10])),
                                           len(VBEPostings.encode_tf([2, 4, 129, 3, 30]))),
                                       2: (len(VBEPostings.encode([2, 3, 4, 8, 10])) + len(VBEPostings.encode_tf([2, 4, 129, 3, 30])),
                                           3,
