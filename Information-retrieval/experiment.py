...
def dcg(ranking):
    ...
    for i in range(len(ranking)):
        rank = i + 1
        score += ranking[i] / (math.log2(1 + rank))
    return score
...
def prec(ranking, k):
    ...
    relevan = 0
    if k < len(ranking):
        relevan = sum(ranking[:k]) / k
    else:
        relevan = sum(ranking) / len(ranking)
    return relevan
...
def ap(ranking):
    ...
    if num_relevant == 0:
        return 0.0
    ap_score = sum(precision_at_i) / num_relevant
...
def eval_retrieval(qrels, query_file="queries.txt", k=1000):
    ...
    with open(query_file) as file:
        ...
        for qline in tqdm(file):
            ...
            for (score, doc) in BSBI_instance.retrieve_bm25(query, k=k, k1=0.2, b=0.75):
                did = int(os.path.splitext(os.path.basename(doc))[0])
                # Alternatif lain:
                # 1. did = int(doc.split("\\")[-1].split(".")[0])
                # 2. did = int(re.search(r'\/.*\/.*\/(.*)\.txt', doc).group(1))
