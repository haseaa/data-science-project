...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        list_of_bytes = []
        while True:
            list_of_bytes.insert(0, number%128)
            if number<128:
                break
            number//=128
        list_of_bytes[-1]+=128
        return array.array('B', list_of_bytes).tobytes()
    ...
    def vb_encode(list_of_numbers):
        ...
        for number in list_of_numbers:
            list_of_bytestream.append(VBEPostings.vb_encode_number(number))
        return b"".join(list_of_bytestream)
    ...
    def encode(postings_list):
        ...
        for i in range(len(postings_list)):
            if i == 0:
                list_of_gap.append(postings_list[i])
            else:
                list_of_gap.append(postings_list[i] - postings_list[i-1])
        return VBEPostings.vb_encode(list_of_gap)
    ...
    def decode(encoded_postings_list):
        ...
        encoded_bytestream = encoded_bytestream.tolist()
        list_of_gap = VBEPostings.vb_decode(encoded_bytestream)
        postings_list = []
        for i in range(len(list_of_gap)):
            if i == 0:
                postings_list.append(list_of_gap[i])
            else:
                postings_list.append(list_of_gap[i] + postings_list[i-1])
...
if __name__ == '__main__':
    ...
    for Postings in [StandardPostings, VBEPostings]:
        ...
        assert decoded_tf_list == tf_list, "hasil decoding tidak 0sama dengan postings original"
        print()
